...
variable.vi <-  which(!(colnames(preclinic_raw.df) %in% ProMetIS:::.varLabels_first.ls()[["preclinical"]]))
...
stopifnot(identical(dim(preclinic_dat.mn), as.integer(c(42, 213))))
```
...
stopifnot(identical(dim(preclinic_fda.df), as.integer(c(213, 3))))
```
...
clinic.eset <- Biobase::ExpressionSet(assayData = t(preclinic_dat.mn),
